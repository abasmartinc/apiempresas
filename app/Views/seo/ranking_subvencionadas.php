...
<head>
    ...
    <style>
        ...
        /* ── PAYWALL BLUR ── */
        .blurred-amount { display: inline-flex; align-items: center; gap: 8px; }
        .blurred-amount .amount-text { filter: blur(5px); user-select: none; color: #059669; font-weight: 800; font-size: 0.9rem; }
        .blurred-amount .lock-btn {
            ...
            white-space: nowrap; box-shadow: 0 2px 8px rgba(16,185,129,0.3); transition: all 0.2s;
        }
        .blurred-amount .lock-btn:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(16,185,129,0.4); }
        .paywall-row { opacity: 0.85; }
        .paywall-row td { background: #fafffe !important; }
        .paywall-cta-banner {
            background: linear-gradient(135deg, #f0fdf9, #ecfdf5); border: 1px solid #a7f3d0;
            border-radius: 16px; padding: 20px 28px; margin: 20px 0 0;
        ...
        .paywall-cta-banner a {
            ...
            box-shadow: 0 4px 14px rgba(16,185,129,0.3); flex-shrink: 0;
        }
...
<body>
    ...
    <header class="dir-hero">
        <div class="container">
            <nav class="dir-breadcrumbs">
                ...
                <span>Empresas más Subvencionadas</span>
            </nav>
    ...
    <main class="dir-main">
        <div class="container">
            <div class="dir-main-card">
                ...
                <form method="GET" action="<?= site_url('empresas-mas-subvencionadas-espana') ?>" style="margin-bottom: 24px; position: relative;">
                ...
                <div style="margin-bottom: 32px; padding: 16px 24px; background: #ecfdf5; border-radius: 14px; border: 1px solid #a7f3d0; display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <a href="<?= site_url('empresas-mas-subvencionadas-espana') ?>" style="color: #64748b; text-decoration: none; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 4px;">
                ...
                <div class="dir-stats">
                    ...
                    <div class="dir-stat">
                        ...
                        <div>
                            ...
                            <div class="dir-stat__label">Subvenciones Concedidas</div>
                        </div>
                    </div>
                    <div class="dir-stat">
                        ...
                        <div>
                            ...
                            <div class="dir-stat__label">Registros de Ayudas</div>
                        </div>
                    </div>
                </div>
                ...
                <section class="dir-section">
                    <div class="section-header">
                        ...
                        <h2>Ranking por Subvenciones Recibidas</h2>
                            ...
                            <?php else: ?>
                                ...
                                <tr<?= (!$isLocked && $hasCompany) ? ' onclick="window.location=\'' . esc($cUrl) . '\'"' : ' style="cursor: default;"' ?><?= $isLocked ? ' class="paywall-row"' : '' ?>>
                                    ...
                                    <td>
                                        <?php if ($hasCompany): ?>
                                        <?php if (!$isLocked): ?>
                                        <a href="<?= esc($cUrl) ?>" style="font-weight: 700; color: #0f172a; text-decoration: none; transition: color 0.2s;" onmouseover="this.style.color='#10b981'" onmouseout="this.style.color='#0f172a'">
                                            <?= esc($co['company_name']) ?>
                                        </a>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <?php if (!$isLocked): ?>
                                        <span style="font-weight: 700; color: #0f172a; display: block; margin-bottom: 6px;"><?= number_format($co['total_subsidies'], 0, ',', '.') ?></span>
                                        <div class="pbar-track"><div class="pbar-fill pbar-fill--green" style="width: <?= $pct ?>%; animation-delay: <?= $delay ?>s;"></div></div>
                                        <?php else: ?>
                                        <span style="filter: blur(4px); font-weight: 700; color: #0f172a; user-select: none;"><?= number_format($co['total_subsidies'], 0, ',', '.') ?></span>
                                        <?php endif; ?>
                                    </td>
                ...
                <section style="margin-top: 48px; padding-top: 40px; border-top: 1px solid #e2e8f0;">
                    <h2 style="font-size: 1.4rem; font-weight: 800; color: #0f172a; margin-bottom: 1.5rem; letter-spacing: -0.025em;">Preguntas Frecuentes sobre las Subvenciones Públicas</h2>
                    <div style="display: grid; gap: 24px;">
                        <div>
                            <h3 style="font-size: 1.1rem; font-weight: 700; color: #1e293b; margin-bottom: 8px;">¿Qué son las subvenciones a empresas?</h3>
                            <p style="color: #475569; font-size: 0.95rem; line-height: 1.6; margin: 0;">Las subvenciones a empresas son ayudas económicas directas o indirectas concedidas por el Estado, Comunidades Autónomas u Organismos Europeos a entidades privadas para fomentar actividades de interés general como la innovación, la contratación o la internacionalización.</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p style="color: #475569; font-size: 0.95rem; line-height: 1.6; margin: 0;">Sí. En virtud de la Ley de Transparencia (Ley 19/2013), toda subvención pública que supere los 3.000 € debe ser publicada en la Base de Datos Nacional de Subvenciones, que es de acceso público y gratuito.</p>
                        </div>
                    </div>
                </section>
                <!-- /SEO Content -->
    ...
    <script type="application/ld+json">
    {
      ...
      "mainEntity": [{
        "@type": "Question",
        "name": "¿Qué son las subvenciones a empresas?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Las subvenciones a empresas son ayudas económicas directas o indirectas concedidas por el Estado, Comunidades Autónomas u Organismos Europeos a entidades privadas para fomentar actividades de interés general."
        }
      },{
        "@type": "Question",
        ...
        "@type": "Question",
        "name": "¿Son públicos los datos de subvenciones a empresas en España?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Sí. En virtud de la Ley de Transparencia (Ley 19/2013), toda subvención pública que supere los 3.000 € debe ser publicada en la Base de Datos Nacional de Subvenciones, de acceso público y gratuito."
        }
      }]
    }
    </script>
