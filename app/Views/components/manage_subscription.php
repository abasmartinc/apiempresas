    ...
    <div style="margin-top: 0px; margin-bottom: 40px; position: relative; z-index: 10;">
        ...
        <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 24px; padding: 40px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.02), 0 4px 6px -4px rgba(0,0,0,0.02);">
            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 32px; border-bottom: 1px solid #f1f5f9; padding-bottom: 24px;">
                <div>
                    <div style="font-size: 0.8rem; font-weight: 800; text-transform: uppercase; color: #64748b; letter-spacing: 0.05em; margin-bottom: 6px;"><?= lang('Billing.manage_billing') ?></div>
                    <h2 style="font-size: 1.5rem; font-weight: 900; color: #0f172a; margin: 0;"><?= lang('Billing.subscription_status') ?></h2>
                </div>
                <?php 
                ...
                <div style="padding: 6px 16px; border-radius: 999px; font-size: 0.85rem; font-weight: 800; display: inline-flex; align-items: center; gap: 6px; <?= $isCanceled ? 'background: #fef2f2; color: #b91c1c; border: 1px solid #fecaca;' : 'background: #ecfdf5; color: #059669; border: 1px solid #a7f3d0;' ?>">
                    ...
                    <?= $isCanceled ? lang('Billing.canceled') : lang('Billing.active') ?>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 24px; margin-bottom: 32px;">
                <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 20px;">
                    <div style="font-size: 0.85rem; font-weight: 700; color: #64748b; margin-bottom: 4px;"><?= lang('Billing.hired_plan') ?></div>
                    <div style="font-size: 1.25rem; font-weight: 900; color: #0f172a;"><?= esc($planName) ?></div>
                </div>
                <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 20px;">
                    <div style="font-size: 0.85rem; font-weight: 700; color: #64748b; margin-bottom: 4px;"><?= lang('Billing.next_charge') ?></div>
                    <div style="font-size: 1.25rem; font-weight: 900; color: #0f172a;"><?= $periodEnd ? esc(date('d/m/Y', strtotime((string) $periodEnd))) : '—' ?></div>
                </div>
                <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 20px;">
                    <div style="font-size: 0.85rem; font-weight: 700; color: #64748b; margin-bottom: 4px;"><?= lang('Billing.payment_method') ?></div>
            ...
            <div style="margin-bottom: 32px;">
                <h3 style="font-size: 1.1rem; font-weight: 800; color: #0f172a; margin-bottom: 16px;"><?= lang('Billing.plan_history') ?></h3>
                <div style="border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
                    <table style="width: 100%; border-collapse: collapse; font-size: 0.95rem;">
                        <thead style="background: #f8fafc; border-bottom: 1px solid #e2e8f0;">
                            <tr style="text-align: left; color: #64748b; font-weight: 700;">
                                <th style="padding: 16px 20px;"><?= lang('Billing.plan') ?></th>
                                <th style="padding: 16px 20px;"><?= lang('Billing.status') ?></th>
                                <th style="padding: 16px 20px;"><?= lang('Billing.end_period') ?></th>
                                <th style="padding: 16px 20px;"><?= lang('Billing.action') ?></th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr style="border-bottom: 1px solid #f1f5f9; background: #ffffff;">
                                    ...
                                    <td style="padding: 16px 20px;">
                                        <span style="display: inline-block; padding: 4px 10px; border-radius: 99px; font-size: 0.8rem; font-weight: 800; background: <?= ($sub->status === 'canceled') ? '#fef2f2; color: #b91c1c;' : '#ecfdf5; color: #059669;' ?>">
                                            <?= ($sub->status === 'canceled') ? lang('Billing.canceled') : lang('Billing.active') ?>
                                        </span>
                                    </td>
                                    ...
                                    <td style="padding: 16px 20px;">
                                        <?php if ($sub->status === 'active'): ?>
                                            <form class="form-cancel-sub-item" action="<?= site_url('billing/cancel-subscription') ?>" method="POST" style="margin: 0;">
                                                ...
                                                <button type="submit" style="background: #fef2f2; color: #dc2626; border: 1px solid #fecaca; padding: 6px 12px; border-radius: 8px; font-size: 0.85rem; font-weight: 700; cursor: pointer; transition: all 0.2s;"><?= lang('Billing.cancel_plan') ?></button>
                                            </form>
                                        <?php else: ?>
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 16px; align-items: center; border-top: 1px solid #e2e8f0; padding-top: 32px;">
                <?php if (!empty($stripe_customer_id)): ?>
                    <a href="<?= site_url('billing/portal') ?>" style="display: inline-flex; align-items: center; gap: 8px; background: #0f172a; color: white; padding: 14px 24px; border-radius: 12px; font-weight: 800; text-decoration: none; font-size: 0.95rem; transition: background 0.2s;">
                        ...
                        <?= lang('Billing.manage_billing_btn') ?>
                    </a>
                <?php endif; ?>

                <a href="<?= site_url('billing/invoices') ?>" style="display: inline-flex; align-items: center; background: #ffffff; border: 1px solid #cbd5e1; color: #334155; padding: 14px 24px; border-radius: 12px; font-weight: 700; text-decoration: none; font-size: 0.95rem; transition: all 0.2s;"><?= lang('Billing.view_invoices') ?></a>

                <?php if (!$isCanceled): ?>
                    <form id="formCancelSubscription" action="<?= site_url('billing/cancel-subscription') ?>" method="POST" style="margin: 0; margin-left: auto;">
                        <?= csrf_field() ?>
                        <button type="submit" style="background: transparent; color: #dc2626; border: none; font-weight: 700; cursor: pointer; font-size: 0.95rem; text-decoration: underline;"><?= lang('Billing.cancel_subscription') ?></button>
                    </form>
                <?php else: ?>
                    <div style="margin-left: auto; font-size: 0.9rem; color: #64748b; font-weight: 600;">
                        <?= lang('Billing.will_end_on') ?> <?= esc(date('d/m/Y', strtotime((string) $periodEnd))) ?>.
                    </div>
                <?php endif; ?>
            </div>
            ...
            <?php if ($planName === 'Pro'): ?>
                <div style="margin-top: 32px; background: linear-gradient(135deg, #2152ff 0%, #1e40af 100%); border-radius: 16px; padding: 24px; color: white; display: flex; align-items: center; justify-content: space-between; gap: 24px; box-shadow: 0 10px 15px -3px rgba(33, 82, 255, 0.3);">
                    <div>
                        <div style="font-size: 1.2rem; font-weight: 900; margin-bottom: 4px;"><?= lang('Billing.need_more_power') ?></div>
                        <div style="font-size: 0.95rem; color: #bfdbfe; font-weight: 500;"><?= lang('Billing.upgrade_business_desc') ?></div>
                    </div>
                    <button style="background: white; color: #1e40af; border: none; padding: 12px 24px; border-radius: 99px; font-weight: 800; font-size: 0.95rem; cursor: pointer; flex-shrink: 0;" onclick="document.getElementById('plan_business').click(); window.scrollTo({top:0, behavior:'smooth'});"><?= lang('Billing.upgrade_business_btn') ?></button>
                </div>
            <?php endif; ?>
