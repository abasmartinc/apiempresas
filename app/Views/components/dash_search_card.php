...
<section class="activation-main-card" data-track-section="activation_search" style="position: relative; <?= $isLimitReached ? 'border: 2px solid #e11d48; background: #fff1f2;' : '' ?>">
    <?php if (!$isPaid): ?>
        <div style="position: absolute; top: -14px; left: 32px; background: #2152ff; color: white; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 900; text-transform: uppercase; letter-spacing: 0.05em; box-shadow: 0 4px 6px rgba(33, 82, 255, 0.3);"><?= lang('Dashboard.step_1') ?></div>
    <?php endif; ?>
    
    <div class="activation-header" style="margin-top: 10px;">
        <h2><?= $isLimitReached ? ($isPaid ? lang('Dashboard.limit_monthly_reached') : lang('Dashboard.limit_reached')) : ($isPaid ? lang('Dashboard.validate_seconds') : lang('Dashboard.test_api_seconds')) ?></h2>
        <p><?= $isLimitReached ? lang('Dashboard.exhausted_queries', [number_format($limitToUse, 0, ',', '.')]) : lang('Dashboard.enter_cif') ?></p>
    </div>
    
    <div class="search-form-dash" style="position: relative;">
        <input type="text" id="dash_q" class="search-input-dash" placeholder="<?= lang('Dashboard.placeholder_cif') ?>" <?= $isLimitReached ? 'readonly style="background: #f8fafc; cursor: not-allowed;"' : '' ?>>
        <button id="btnDashValidate" class="btn-validate-dash" <?= $isLimitReached ? 'style="background: #94a3b8;"' : '' ?>><?= lang('Dashboard.validate_now') ?></button>
    ...
    <div style="margin-top: 12px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
        <span style="font-size: 0.75rem; color: #94a3b8; font-weight: 700;"><?= lang('Dashboard.test_with') ?></span>
    ...
    <div class="progress-container" style="margin-top: 12px;">
        ...
        <span class="progress-text" style="font-size: 0.8rem; font-weight: 700; color: #64748b; margin-top: 8px; display: block;">
            <?= lang($isPaid ? 'Dashboard.queries_used' : 'Dashboard.companies_tested', [number_format($requestsUsed, 0, ',', '.'), number_format($limitToUse, 0, ',', '.')]) ?>
        </span>
        <span style="font-size: 0.75rem; color: #64748b; font-weight: 700; display: block; margin-top: 4px;">
            <?php if ($isLimitReached): ?>
                <?= $isPaid ? lang('Dashboard.reached_plan_limit') : lang('Dashboard.reached_limit') ?>
            <?php else: ?>
                <?= $isPaid ? lang('Dashboard.current_usage') : lang('Dashboard.complete_validations') ?>
            <?php endif; ?>
        </span>
    </div>
    <?php endif; ?>
    ...
    <div style="margin-top: 16px; font-size: 0.85rem; color: #64748b; font-weight: 600; display: flex; align-items: center; gap: 8px;">
        ...
        <span><?= lang('Dashboard.real_case') ?></span>
    </div>
    ...
    <div class="aha-moment-card" id="aha-moment-card">
        <div class="aha-header">
            ...
            <?= lang('Dashboard.company_found') ?>
        </div>
        <div class="aha-grid">
            <div class="aha-item" style="grid-column: span 2;">
                <div class="aha-label"><?= lang('Dashboard.name') ?></div>
                <div class="aha-val" id="aha-name" style="font-size: 1.1rem; font-weight: 800;">-</div>
            </div>
            <div class="aha-item">
                <div class="aha-label"><?= lang('Dashboard.cif') ?></div>
                <div class="aha-val" id="aha-cif">-</div>
            </div>
            <div class="aha-item">
                <div class="aha-label"><?= lang('Dashboard.status') ?></div>
        ...
        <div style="margin-top: 20px; border-top: 1px solid #bae6fd; padding-top: 16px;">
            <p style="font-size: 0.85rem; color: #0369a1; font-weight: 700; margin-bottom: 16px;"><?= lang('Dashboard.integrate_crm') ?></p>
    ...
    <div id="inlineJsonContainer" style="display: none; margin-top: 16px; animation: slideDown 0.4s ease-out;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <span style="font-size: 0.75rem; font-weight: 800; color: #64748b; text-transform: uppercase;"><?= lang('Dashboard.json_response') ?></span>
            <button id="btnCopyInlineJson" style="background: none; border: none; color: #2152ff; font-weight: 800; font-size: 0.7rem; cursor: pointer;"><?= lang('Dashboard.copy_json') ?></button>
        </div>
