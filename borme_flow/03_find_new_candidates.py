...
def run():
    ...
    try:
        with conn.cursor() as cur:
            ...
            query = "SELECT p.id, p.company_name, p.borme_date FROM borme_posts p LEFT JOIN borme_candidates c ON c.post_id = p.id WHERE p.company_id IS NULL AND p.candidate_processed = 0 AND c.id IS NULL ORDER BY p.id DESC LIMIT 300000"
            ...
            for it in rows:
                ...
                if not cur.fetchone():
                    cur.execute("INSERT INTO borme_candidates (post_id, raw_name, normalized_name, borme_date) VALUES (%s,%s,%s,%s)", (it["id"], it["company_name"], nm, it["borme_date"]))
                    inserted_count += 1
